...
| Xbox keys without a generation ("<Game> XBOX LIVE Key EUROPE", "(Xbox Live)", Gamivo `-xbox-xbox-windows-`) | **disponible** since 2026-09-25 | P4 decided by Romain (« Xbox sur les deux »): read as Xbox One + Series, entered on the pages AKS has (both missing → `no AKS product page found (console)`); + PC / Windows = the P2 Play Anywhere case (confirmed 2026-09-26: « Xbox + PC reste playanywhere »); since 2026-09-26 (« les 2 ») the Xbox is also read in the URL (etailcard / lootbar bare `xbox`, Gamivo `-xbox-<cc>`). PlayStation / Switch without a generation stay refused `console: no declared generation (R45)` | EXECUTOR_RULES §4.12.3 / P4 |
| Windows / Xbox app keys ("(Windows) XBOX LIVE Key", "PC/XBOX LIVE Key", "Windows 11/Xbox Live Key", Gamivo `-xbox-pc-`) | **disponible** since 2026-09-26 | Romain's « 1. »: the AKS PC page decides — « Xbox Play Anywhere » → the Play Anywhere targets (Xbox pages AKS has + PC page, XBOX/PC buckets); else « Microsoft Windows » → a Microsoft Store key (`MICROSOFT`, Windows 10 buckets 246 / 244 / 245 / 249) on the PC page; else refused. Never P2 on the title alone. Was refused `PC-only Xbox Live key` | EXECUTOR_RULES §4.12.3 / §4.12.4 (f bis) |
...
- **Console keys — multi-target candidates, live** `[R45]` (2026-09-12 → 15). AKS has
  ...
  day; on 2026-09-26 P2 was confirmed, the Xbox is also read in the URL (« les 2 »), and a
  Windows / Xbox app key ("(Windows) XBOX LIVE Key") is entered by what the AKS PC page says
  (Play Anywhere, else Microsoft Store, else refused). Rules:
...
trois fois. Un doute APRÈS un clic reste une halte immédiate (EXECUTOR_RULES §14). Depuis le
2026-09-26, cela couvre aussi un contrôle de connexion ou un catalogue muet avant la première
offre (Eneba p66) et une ré-extraction qui crashe sur un timeout CDP (Gamivo p38) — effectif au
prochain lancement d'un balayage.
