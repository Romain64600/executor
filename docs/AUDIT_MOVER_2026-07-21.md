...
### MV1. Move de la MAUVAISE offre si un re-import ré-ide la row (garde SC5 manquant) — OPEN
...
### MV2. Tout blocker `_locate_row` avalé en SKIP « déjà déplacé » — OPEN
...
### MV3. `build_move_plan` admet une URL marchande vide → preuve de disparition id-seule (falsifiable) — OPEN
...
### MV4. Le DRY-RUN (défaut) s'auto-bloque au-delà de 10 offres — OPEN
...
### MV5. `resolve_list_id` prend le 1er match de label sans garde d'ambiguïté — OPEN
...
### MV6. `--execute --mode safe` par défaut = plan COMPLET sur un writer jamais testé live — OPEN (décision)
...
### MV7. Pas de settle entre l'Apply (POST natif) et le re-scan de vérif — OPEN
...
### MV8. Index de départ early-terminé (2 pages sans nouveaux ids) → offre en page ultérieure = faux « absent » — OPEN
...
### MV9. Stop `guard.blocked` manquant après `record_result` — OPEN
...
### MV10. Overrides `--store-id`/`--source-list` non réconciliés avec raw.json — OPEN
Un mismatch dégrade en faux « already moved » silencieux. **Fix** : si un override
diffère de `plan_doc`, refuser (fail-closed).

### MV11. Exception CDP dans `set_bulk_list`/`click_apply` échappe l'abort structuré — OPEN
...
### MV12. Post-verify prouve le DÉPART de la source, pas l'ARRIVÉE sur la cible — OPEN (doc + option)
...
### MV13. Cohérence learning.json ↔ skipped.json non vérifiée + docstrings surévaluées — OPEN
...
### MV14. `resolve_list_id` ignore `target_id_hint` alors que la docstring promet un fallback — OPEN
**Fix** : label-only fail-closed — retirer le param inutilisé + corriger la doc.

### MV15. Tests manquants (pré-canary) — OPEN
...
- **MV6** : politique du 1er move — imposer un canary (`--mode learning`) tant
  qu'aucun move réussi n'est au ledger ? (recommandé, fail-closed) → OUVERTE
- **MV12** : ajouter une confirmation de présence sur la liste cible (double le
  scan par offre) ou se contenter de « gone from source » + doc ? → OUVERTE
